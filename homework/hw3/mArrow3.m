function [h, coordinateStructure] = mArrow3(p1,p2,varargin)
...
for argno = 1:2:nargin-2
    switch varargin{argno}
        ...
        case  'returnCoordinates'
            % set flag for coordinate return
            returnCoordinates = 1;
...
%     end
    ...
    v  = stemWidth*(sintheta* y'  + costheta* z') + repmat(p1',[ppsc+1 1]) ; 
...
%     end
    ...
    v = [v; stemWidth*(sintheta* y'  + costheta* z') + repmat(p3',[ppsc+1 1])] ; 
...
%     end
    ...
    v = [v;  tipWidth*(sinupsilon* y'  + cosupsilon* z') + repmat(p3',[ppbc+1 1])];  
...
%     end
    f(2:ppsc+1,1:4) = repmat((1:ppsc)',[1 4]) + repmat([0 1 ppsc+2 ppsc+1], [ppsc 1]) ;  
...
%     end
    f(ppsc+3:ppsc+2+ppbc,1:3) = repmat([2*ppsc+2  2*ppsc+3  2*ppsc+ppbc+4],[ppbc 1]) + [(1:ppbc)' (1:ppbc)' zeros(ppbc,1) ] ; 
...
%     end
    v  = tipWidth*(sinupsilon* y'  + cosupsilon* z') + repmat(p1',[ppbc+1 1]) ; 
...
%     end
    f(2:ppbc+1,1:3) = repmat([0 1 ppbc+2],[ppbc 1]) + ([(1:ppbc)' (1:ppbc)' zeros(ppbc,1)]) ; 
...
if ~returnCoordinates
    ...
    % nargout gets coordinates 
    coordinateStructure.Xdata
    coordinateStructure.Ydata
    coordinateStructure.Zdata
end